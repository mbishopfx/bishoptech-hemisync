...
- `components/machine/ToneMachineDemo.jsx` registers the thirteen homepage tools with `document.modelContext.registerTool`.
...
- `app/api/agent/tone-compare`, `app/api/agent/session-plan`, and `app/api/agent/session-cue` provide the REST fallback for those same session journeys.
...
Use a compatible ChatGPT in-app browser or Chrome with WebMCP enabled. For a reproducible local Chrome run, open `chrome://flags/#enable-webmcp-testing`, set the flag to Enabled, relaunch Chrome, and visit the live homepage. The homepage should expose thirteen browser tools: state, absolute controls, relative carrier nudge, intention matching, direction comparison, session planning, session cues, pack search, pack preview, policy, account options, generic preview, and signup navigation. The public MCP endpoint should expose twenty-one tools, including `get_ios_app_offer`, `compare_tone_directions`, `plan_listening_session`, and `get_session_cue`. Confirm that the iPhone offer returns the App Store URL and $2.99 one-time price without a payment side effect. Confirm that intention matching selects a public tone, a relaxation search returns a published pack, an unconfirmed pack preview returns `CONFIRMATION_REQUIRED`, a 20-minute plan returns arrive/practice/close phases, a cue does not echo diary content, and a gamma/246 Hz control call is reflected in the visible state. In the connected ChatGPT app, ask “open the tone machine for gamma at 246 Hz” and verify that the `text/html;profile=mcp-app` widget renders the Aurora visual, shows Gamma/246 Hz, offers the session guidance actions, and leaves audio off until the play button is pressed. Start at the live homepage and follow the short recording script in `DEMO-SCRIPT.md`. A normal browser should still provide the complete human controls without either agent surface.
