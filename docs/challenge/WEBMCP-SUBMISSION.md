...
Use a compatible ChatGPT in-app browser or Chrome with WebMCP enabled and visit `/try`. The cockpit should expose 28 public browser tools, including five Agentic Session Score actions: compose, refine a stage or sound profile, undo, select, and confirmed preview. Ask for a 10-minute focus score; verify three visible stages totaling exactly 600 seconds, constant carrier inside each stage, and a linear 0.1–40 Hz differential path. Select stage 2, refine it to a 2,000 Hz carrier and 0.1→39.9 Hz differential, then undo. Toggle monaural and isochronic modes, add the approved ocean ambience metadata, enable a breath pattern, set fades, and export the technical JSON. An unconfirmed score preview must return `CONFIRMATION_REQUIRED`; a confirmed preview may claim playback only after `audioReady: true`, reports the active signal modes, and remains capped at 120 seconds. The public MCP endpoint should expose 38 tools including read-only `compose_session_score`; its output must state that persistence, rendering, and audio are false while returning the full-spectrum sound profile. The authenticated member bridge remains eleven bounded tools. Continue through the existing machine, science, account, feedback, and no-credential payment checks. A normal browser still provides the complete human score and machine controls without WebMCP.
