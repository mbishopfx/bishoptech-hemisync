...
| `https://cognistration.com/api/capabilities` | pass | production manifest returns 19 homepage WebMCP tools, 11 authenticated member tools, 29 public MCP tools, 15 MCP resources, and the science-guide/tone-pack commerce capabilities |
| `https://cognistration.com/openapi.json` | pass | generated REST compatibility document mirrors the approved 29-tool public registry, includes `/api/machine-payments/tone-pack`, and exposes no write credentials |
...
| `https://cognistration.com/api/mcp` | pass | live discovery and tool/resource checks return 29 public tools, 15 resources, the skills extension, the science guide, and the tone-pack checkout resource |
| `https://cognistration.com/try` connection helper | pass | production browser flow exposes the copyable remote-app setup prompt, frosted step cards, current 29-tool count, and expanded Plugins-tab guidance; no URL is sent to a Git installer |
...
Current state after the MCP Apps machine, frequency-wave visual, iPhone offer, ChatGPT connection helper, homepage presentation, five-skill catalog, in-platform account capture, in-platform done-state feedback, frosted cockpit polish, production science guide, and tone-pack commerce card: `production-public-authenticated-member-native-webmcp-mcpapps-ios-offer-chatgpt-setup-helper-homepage-polish-frequency-wave-science-guide-canvas-tone-pack-checkout-mpp-verified`. Vercel deployment `dpl_GJUU1fEMP66sTtbGQYnWz9qD3JJA` is ready on commit `77ff9ae` with the `cognistration.com` alias. Live verification covers 29 MCP tools, 15 resources, 19 homepage WebMCP tools, the seven-slide science widget with a self-contained animated ocean canvas and no embedded source page, the frosted tone-pack checkout resource, and enabled fixed $0.50 and $5.99 payment-option routes. No real payment was initiated during verification; the payment coda must still be shown only with the existing user-approved provider client.
