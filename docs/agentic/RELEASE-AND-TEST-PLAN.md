...
| `POST /api/mcp` `get_tone_pack_payment_options` | pass | production advertises the fixed 599-cent USD MPP route, the default `Authorization: Payment` credential plus `Payment-Authorization` compatibility header, `Payment-Receipt`, email-required delivery, and no payment-details field |
