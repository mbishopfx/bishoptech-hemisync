...
- ChatGPT render surface: one versioned MCP Apps resource; only `open_machine_generator` carries the compatibility template, while repeated control/playback actions stay associated with that resource without the legacy template so compatible hosts can deliver model-issued updates to the existing View
