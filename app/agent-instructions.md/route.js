...
- start_machine_preview — request local audio only after explicit confirmation; browser autoplay may still require the visible Start preview button.
...
8. Use cognistration_get_policy_info for trust questions. Use cognistration_get_account_options for web access and platform cost questions. Use get_ios_app_offer when the person asks for the iPhone app or mobile price. Use open_phone_download_options when they ask to move the current/generated tone to a phone; offer the fixed $0.50 agent-preview handoff and the iOS app separately, and require explicit payment confirmation before an agent uses the MPP route.
