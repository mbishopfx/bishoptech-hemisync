...
Cognistration is a public audio-session product. The homepage exposes a native WebMCP bridge when the browser supports document.modelContext.registerTool, and the public MCP endpoint can render interactive tone, science-guide, tone-pack checkout, account-signup, and feedback widgets inside a compatible ChatGPT app host. The bridges can read the visible session machine, set bounded controls, match a short intention to a public tone, clarify vague requests, calibrate controls from listener feedback, compare directions, conduct an arrive → practice → close ritual, prepare a technical-settings-only recipe, explain the signal and FFR evidence boundaries in a click-through guide, plan a session, return a small cue, browse tone packs, preview a selected pack, read policy information, return the iPhone app offer, render the user-controlled signup and tone-pack purchase forms, and open an optional done-state feedback card. Starting audio requires explicit confirmation; checkout, signup, feedback submission, and App Store payment remain user-controlled.
...
When a person asks to open the machine in ChatGPT, call open_machine_generator. The returned MCP Apps widget uses the versioned resource ui://cognistration/machine-generator/v1.html, can call recommend_tone, search_public_tone_packs, compare_tone_directions, plan_listening_session, and get_session_cue through the app bridge, and never starts audio automatically. When a person asks to buy a pack, call open_tone_pack_checkout; the versioned tone-pack widget keeps email entry and explicit confirmation in-platform, opens hosted Checkout, and renders a download action only from the verified delivery result. When a person asks to create an account, call open_account_signup; the versioned account widget keeps credential entry and submission in-platform and never starts checkout. When a person says they are done, offer open_feedback once. The versioned feedback widget keeps the optional note out of MCP arguments and has no history view. prepare_session_recipe remains available as a direct remote MCP tool and as a browser WebMCP action on the full page. If a host cannot render custom UI, return the structured result and explain the bounded fallback; do not automatically send the person away from the current surface.
