...
- compose_session_score — validate or deterministically compose a one-to-six-stage technical score with constant per-stage carrier and a truthful linear beat path. It has no persistence, render, or audio side effect.
...
4. For a multi-stage technical plan, use cognistration_compose_session_score on /try. Refine and select only visible stage IDs, and use undo for browser-local revisions. Never describe carrier steps as a continuous sweep; only the beat path is linear inside a stage.
