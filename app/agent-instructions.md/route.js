...
- open_machine_generator — call this Cognistration MCP tool to render the interactive tone machine in a compatible ChatGPT app host; do not use computer-use or browser navigation for the machine. Optional intention, public tone ID, state, carrier, rhythm, and volume inputs seed the widget.
...
For machine rendering and control, prefer the remote Cognistration MCP connector directly. Do not invoke computer-use, browser automation, or website navigation for an MCP machine request; browser WebMCP is only the fallback for an agent explicitly operating the /try page.
