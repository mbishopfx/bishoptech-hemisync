...
Cognistration is a public audio-session product. The homepage exposes a native WebMCP bridge when the browser supports document.modelContext.registerTool, and the public MCP endpoint can render interactive tone, account-signup, and feedback widgets inside a compatible ChatGPT app host. The bridges can read the visible session machine, set bounded controls, match a short intention to a public tone, clarify vague requests, calibrate controls from listener feedback, compare directions, conduct an arrive → practice → close ritual, prepare a technical-settings-only recipe, plan a session, return a small cue, browse tone packs, preview a selected pack, read policy information, return the iPhone app offer, render the user-controlled signup form, and open an optional done-state feedback card. Starting audio requires explicit confirmation, and signup, feedback submission, and App Store payment always remain user-controlled.
...
8. On the current browser page, offer cognistration_open_account_signup if the user wants to create an account; for remote MCP, call open_account_signup so the form stays in the agent surface. Leave credential, verification, sign-in, and payment submission to the user.
9. When the listener says they are done, offer one optional open_feedback check-in. Do not submit or echo the rating or note through MCP.
