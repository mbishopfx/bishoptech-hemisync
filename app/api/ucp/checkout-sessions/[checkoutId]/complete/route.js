...
import { completeUcpCheckout } from '@/lib/commerce/ucp.mjs';
...
export async function POST(request, { params }) {
  if (commerceRateLimited(request, { scope: 'ucp-complete-checkout', limit: 20 })) {
  ...
  try {
    ...
    if (!rawKey) throw commerceError('IDEMPOTENCY_REQUIRED', 'Idempotency-Key is required for checkout completion.', 400);
    const result = await completeUcpCheckout({
