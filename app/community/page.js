...
export default async function CommunityPage() {
  ...
  const creatorHighlights = creatorRollup
  ...
  const canFollowFeatured = Boolean(
    currentUser &&
      ...
      currentUser.id !== featuredCreator.profile.id,
  );
  ...
  const communityStats = [
    ...
    {
      label: "Tone shares",
      value: feed.filter((post) => post.saved_tones).length.toString(),
    },
  ];
      ...
      <div className="max-w-3xl mx-auto w-full flex flex-col gap-8">
        ...
        <Card className="p-6 bg-[var(--card-bg)] shadow-premium border-none">
          <div className="flex flex-col gap-5 lg:flex-row lg:items-center lg:justify-between">
            <div className="flex items-start gap-4">
              ...
              <div>
                ...
                {featuredCreator ? (
                  <>
                    <Link
                      href={`/community/${featuredCreator.profile?.username || featuredCreator.profile?.id}`}
                      className="mt-2 block text-2xl font-display text-foreground hover:underline"
                    >
                      {featuredCreator.profile?.display_name || "Member"}
                    </Link>
                    <p className="mt-1 text-sm text-muted">
                      @{featuredCreator.profile?.username || "member"} •{" "}
                      {featuredCreator.postCount} public post
                      {featuredCreator.postCount === 1 ? "" : "s"} •{" "}
                      {featuredCreator.toneCount} tone share
                      {featuredCreator.toneCount === 1 ? "" : "s"}
                    </p>
                  </>
                ) : (
                  <>
                    ...
                    <p className="mt-1 text-sm text-muted">
                      As soon as the first public posts land, this spotlight
                      will highlight the most active member.
                    </p>
                  </>
                )}
              </div>
            ...
            {canFollowFeatured && featuredCreator && (
              <FollowButton
        ...
        <Card className="p-6 bg-[var(--card-bg)] shadow-premium border-none">
          <div className="flex flex-col gap-6">
            <div>
              ...
              <p className="mt-2 text-sm text-muted">
                A rolling snapshot of the latest public activity and creator
                momentum.
              </p>
            </div>
            ...
            <div className="grid gap-3 sm:grid-cols-3">
              {communityStats.map((stat) => (
                <div
                  key={stat.label}
                  className="rounded-2xl border border-[var(--line-soft)] bg-[var(--bg-1)] px-4 py-3"
                >
            ...
            <div className="rounded-2xl border border-[var(--line-soft)] bg-[var(--bg-0)] px-4 py-4">
              <div className="flex items-center justify-between gap-4">
                <div>
                  ...
                  <p className="mt-1 text-sm text-muted">
                    The most shared emotional targets across public tone posts.
                  </p>
                </div>
                <span className="rounded-full border border-[var(--line-soft)] bg-[var(--bg-1)] px-3 py-1 text-[10px] uppercase tracking-[0.25em] text-muted">
              ...
              {toneStateRollup.length === 0 ? (
                <p className="mt-4 text-sm text-muted">
                  State trends will appear once members share public tones.
                </p>
              ) : (
            ...
            <div>
              ...
              {creatorHighlights.length === 0 ? (
                <p className="mt-3 text-sm text-muted">
                  Creator highlights will appear once public posts start flowing
                  in.
                </p>
              ) : (
                <div className="mt-4 grid gap-3">
                  {creatorHighlights.map((creator, index) => (
                    <div
                      key={creator.profile?.id || index}
                      className="flex items-center justify-between rounded-2xl border border-[var(--line-soft)] bg-[var(--bg-0)] px-4 py-3"
                    >
                      <div className="min-w-0">
                        <Link
                          href={`/community/${creator.profile?.username || creator.profile?.id}`}
                          className="block truncate font-medium text-foreground hover:underline"
                        >
                          {creator.profile?.display_name || "Member"}
                        </Link>
                        <p className="truncate text-xs text-muted">
                          @{creator.profile?.username || "member"} •{" "}
                          {creator.toneCount} tone share
                          {creator.toneCount === 1 ? "" : "s"}
                        </p>
                      </div>
        ...
        <Card className="p-6 bg-[var(--card-bg)] shadow-premium border-none">
          <div className="flex items-center justify-between gap-4">
            <div>
              ...
              <p className="mt-2 text-sm text-muted">
                Most resonant posts in the last 100 public shares, ranked by
                likes and comments.
              </p>
            </div>
            <span className="rounded-full border border-[var(--line-soft)] bg-[var(--bg-1)] px-3 py-1 text-[10px] uppercase tracking-[0.25em] text-muted">
          ...
          {trendingPosts.length === 0 ? (
            <p className="mt-4 text-sm text-muted">
              Trending posts will appear here once members start reacting to
              public shares.
            </p>
          ) : (
            <div className="mt-4 grid gap-3">
              {trendingPosts.map((post) => (
                <div
                  key={post.id}
                  className="rounded-2xl border border-[var(--line-soft)] bg-[var(--bg-0)] px-4 py-3"
                >
                  <div className="flex items-start justify-between gap-4">
                    <div className="min-w-0">
                      <Link
                        href={`/community/${post.profiles?.username || post.profiles?.id}`}
                      ...
                      <p className="mt-1 text-xs text-muted line-clamp-2">
                        {post.body ||
                          "Shared a tone update with the community."}
                      </p>
                    </div>
                    <div className="flex shrink-0 flex-col items-end gap-2 text-[10px] uppercase tracking-[0.25em] text-muted">
                      <span className="inline-flex items-center gap-1 rounded-full border border-[var(--line-soft)] bg-[var(--bg-1)] px-2 py-1">
                        <Heart className="size-3 text-[var(--accent-gold-strong)]" />{" "}
                        {post.like_count}
                      </span>
                      <span className="inline-flex items-center gap-1 rounded-full border border-[var(--line-soft)] bg-[var(--bg-1)] px-2 py-1">
        ...
        <div className="flex flex-col gap-6">
          {feed.length === 0 ? (
            <p className="text-muted">
              No public posts yet. Join the platform and be the first to share a
              tone!
            </p>
          ) : (
            feed.map((post) => (
              <Card
                key={post.id}
                className="p-6 bg-[var(--card-bg)] shadow-premium"
              >
                  ...
                  <div className="flex-1">
                    ...
                    {post.saved_tones && (
                      <Card className="mt-4 p-4 bg-[var(--bg-1)] border-none shadow-premium flex items-center justify-between">
                        <div>
                          <span className="text-[10px] font-bold uppercase tracking-wider text-[var(--accent-gold-strong)]">
                            Attached Tone • {post.saved_tones.target_state}
                          </span>
                          <strong className="block mt-1 text-foreground">
                            {post.saved_tones.name}
                          </strong>
                          <span className="text-xs text-muted block mt-1">
                            {post.saved_tones.duration_sec
                              ? Math.floor(post.saved_tones.duration_sec / 60) +
                                " min"
                              : "Unknown duration"}{" "}
                            • {post.saved_tones.base_freq_hz} Hz
                          </span>
                        </div>
                        {post.saved_tones.mp3_url && (
                          <audio
                            controls
                            src={post.saved_tones.mp3_url}
                            className="h-8 w-48"
                            preload="none"
                          />
                        )}
