...
- The public MCP render tool open_machine_generator opens a versioned interactive machine widget with the supplied Aurora visual, bounded controls, public pack browsing, and an explicit local audio preview button. The companion MCP commands get_machine_control_contract, set_machine_controls, adjust_machine_controls, set_machine_direction, start_machine_preview, stop_machine_preview, and open_machine_fullscreen control that same widget; relative changes such as “speed it up” update the existing audio nodes without pausing playback, while browser autoplay may still require a visible user gesture. A start request is not an audible-playback confirmation: the widget must report audioReady=true.
