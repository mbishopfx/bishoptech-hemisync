...
const machineControlOperationOutput = {
  ...
  required: ['capabilityId', 'version', 'status', 'message'],
  additionalProperties: false
};
...
// The open tool owns the compatibility output template and is the only tool
// that should mount a new machine view. Actions keep the same resource
// association (required by MCP Apps hosts to route a model-issued result to
// the active view) but deliberately omit openai/outputTemplate so legacy
// hosts do not treat every control change as a new render request.
const MACHINE_WIDGET_RENDER_TOOL_META = {
  ui: { resourceUri: MACHINE_WIDGET_RESOURCE_URI, visibility: ['model', 'app'] },
  'openai/outputTemplate': MACHINE_WIDGET_RESOURCE_URI,
  'openai/widgetAccessible': true
};

const MACHINE_WIDGET_ACTION_TOOL_META = {
  ui: { resourceUri: MACHINE_WIDGET_RESOURCE_URI, visibility: ['model', 'app'] },
  'openai/widgetAccessible': true
};
...
export const MCP_TOOLS = [
  ...
  },
  {
    ...
    consent: 'none',
    _meta: {
      ...MACHINE_WIDGET_ACTION_TOOL_META,
  ...
  },
  {
    ...
    sideEffect: 'none',
    _meta: {
      ...MACHINE_WIDGET_RENDER_TOOL_META,
