...
const sessionScoreInput = {
  type: 'object',
  properties: { durationSec: { type: 'integer', minimum: 60, maximum: 3600 }, stages: { type: 'array', minItems: 1, maxItems: 6, items: sessionScoreStageInput } },
...
const sessionScoreOutput = {
  ...
  properties: {
    ...
    stages: { type: 'array', minItems: 1, maxItems: 6, items: sessionScoreStageOutput },
    preview: {
      type: 'object',
      properties: {
  ...
  required: ['capabilityId', 'version', 'correlationId', 'status', 'engine', 'durationSec', 'stages', 'preview', 'boundaries'],
  additionalProperties: false
};
...
export const MCP_TOOLS = [
  ...
  },
  {
    ...
    description: 'Validate or deterministically compose a public, technical-only binaural score with one to six stages. Carrier is constant within each stage; beat may move linearly. This read-only tool never saves, renders, or starts audio.',
    inputSchema: {
      type: 'object',
      properties: {
