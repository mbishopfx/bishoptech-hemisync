...
export const MCP_TOOLS = [
  ...
  },
  {
    ...
    description: 'Use this when the user asks whether Cognistration has an iPhone app, wants the mobile download link, or asks about the lower-cost mobile option. Render the in-platform iPhone offer with real app screenshots and a Download Now App Store badge, while also returning the canonical listing, current one-time price, full-access details, and compatibility. Do not process payment or claim a purchase was completed.',
    inputSchema: { type: 'object', properties: {}, additionalProperties: false },
  ...
  },
  {
    ...
    description: 'Use this when a listener asks to download the current or generated tone to their phone, take it to mobile, or continue on a phone. Render two clear options: a fixed $0.50 no-account agent-to-agent phone preview that requires explicit confirmation before payment, and the full $2.99 one-time iPhone App Store download. This render tool never charges, accepts payment credentials, or releases audio by itself.',
    inputSchema: {
      type: 'object',
      properties: {
  ...
  },
  {
    ...
    description: 'Use this after a hosted tone-pack checkout reports payment complete. It verifies the Stripe session and pack slug before returning the direct download URL, protected fallback, email fallback, and public tone-pack web URL; compatible hosts can render the returned download action in the same app card.',
    inputSchema: {
      type: 'object',
      properties: {
  ...
  },
  {
    ...
    description: 'Use this when the user wants to buy a complete Cognistration tone pack. It opens a frosted in-platform purchase card that asks for a delivery email, requires explicit confirmation of the $5.99 one-time price, sends the user to hosted Checkout, and can reveal the verified download button after payment.',
    inputSchema: {
      type: 'object',
      properties: {
  ...
  },
  {
    ...
    description: 'Set one or more bounded visible machine controls in the already-open Cognistration widget. Use this for exact slider values. Existing audio keeps playing and the widget applies the patch to its current oscillator and gain nodes without pausing or restarting them.',
    inputSchema: {
      type: 'object',
      properties: {
  ...
  },
  {
    ...
    description: 'Request local browser audio in the already-open machine after the listener explicitly confirms they want playback. The tool never navigates away. Browser autoplay policy may still require the listener to press the visible Start preview button; if so, the widget reports that honestly.',
    inputSchema: {
      type: 'object',
      properties: {
  ...
  },
  {
    ...
    description: 'Use this after a tone or machine result when the listener wants a concise, click-through explanation of the two-channel signal, FFR, descriptive frequency bands, evidence limits, and safe listening. It opens an educational guide without starting audio, saving a record, or carrying diary text.',
    inputSchema: {
      type: 'object',
      properties: {
        toneId: { type: 'string', minLength: 1, maxLength: 120, description: 'Optional ID from the public tone catalog.' },
