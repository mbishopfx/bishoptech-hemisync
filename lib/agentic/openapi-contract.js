...
const sessionScoreSoundSchema = {
  ...
  properties: {
    ...
    background: { type: 'object', properties: { type: { type: 'string', enum: ['none', 'ocean', 'asset'] }, assetId: { type: 'string', enum: ['lumina', 'mindsEyes', 'nattkatt', 'papa', 'scatter'] }, mixDb: { type: 'number', minimum: -60, maximum: -6 }, crossfadeSec: { type: 'number', minimum: 0, maximum: 10 } }, required: ['type'], additionalProperties: false },
...
const sessionScoreStageOutputSchema = {
  type: 'object',
  properties: {
    ...sessionScoreStageSchema.properties,
    carrierBehavior: { type: 'string', const: 'constant-within-stage' },
    beatBehavior: { type: 'string', enum: ['constant', 'linear-within-stage'] }
  },
  required: [...sessionScoreStageSchema.required, 'carrierBehavior', 'beatBehavior'],
  additionalProperties: false
};
...
const sessionScoreSchema = {
  type: 'object',
  properties: {
    capabilityId: { type: 'string', const: 'cognistration-session-score' },
    version: { type: 'string' },
    correlationId: { type: 'string' },
    ...
    stages: { type: 'array', minItems: 1, maxItems: 12, items: sessionScoreStageOutputSchema },
    preview: {
      type: 'object',
      properties: {
  ...
  required: ['capabilityId', 'version', 'correlationId', 'status', 'engine', 'durationSec', 'sound', 'stages', 'preview', 'boundaries'], additionalProperties: false
};

const sessionScoreRequest = {
  type: 'object',
  properties: {
    intention: { type: 'string', minLength: 1, maxLength: 240 },
    ...
    score: { type: 'object', properties: { durationSec: { type: 'integer', minimum: 60, maximum: 3600 }, stages: { type: 'array', minItems: 1, maxItems: 12, items: sessionScoreStageSchema }, sound: sessionScoreSoundSchema }, required: ['durationSec', 'stages'], additionalProperties: false },
    sound: sessionScoreSoundSchema
  },
  additionalProperties: false
};
...
export function publicOpenApiDocument(origin = 'https://cognistration.com') {
  ...
  return {
    ...
    paths: {
      ...
      '/api/agent/session-score': {
        post: {
          operationId: 'composePublicSessionScore',
          summary: 'Compose or validate a public Agentic Session Score',
          description: 'Returns a technical-only, browser-local full-spectrum score with up to twelve stages. Each stage keeps a constant 50–2,000 Hz carrier while its 0.1–40 Hz differential may change linearly; the score can select signal modes, breath pacing, approved ambience metadata, and fades. No persistence, rendering, or audio side effect occurs.',
          requestBody: { required: false, content: { 'application/json': { schema: sessionScoreRequest } } },
          responses: {
            200: jsonResponse('A bounded ephemeral score or safety redirect.', { type: 'object', required: ['ok', 'score'], properties: { ok: { type: 'boolean' }, score: { anyOf: [{ $ref: '#/components/schemas/SessionScore' }, { $ref: '#/components/schemas/SafetyRedirect' }] } }, additionalProperties: false }),
            400: jsonResponse('Stage counts, totals, or technical values are invalid.', { $ref: '#/components/schemas/Error' }),
            413: jsonResponse('The score request exceeds the public body limit.', { $ref: '#/components/schemas/Error' }),
            503: jsonResponse('The score composer is temporarily unavailable.', { $ref: '#/components/schemas/Error' })
          }
        }
      },
      '/api/packs': {
