...
const sessionScoreStageSchema = {
  ...
  properties: {
    ...
    carrierHz: { type: 'integer', minimum: 100, maximum: 400 },
    beatHz: { type: 'object', properties: { from: { type: 'number', minimum: 0.5, maximum: 40, multipleOf: 0.5 }, to: { type: 'number', minimum: 0.5, maximum: 40, multipleOf: 0.5 } }, required: ['from', 'to'], additionalProperties: false },
    volume: { type: 'integer', minimum: 0, maximum: 100 }
  },
...
const sessionScoreSchema = {
  ...
  properties: {
    ...
    engine: { type: 'string', const: 'browser-binaural-sine' },
    direction: { type: 'string', enum: ['rest', 'reflect', 'focus', 'momentum', 'synthesis'] },
    durationSec: { type: 'integer', minimum: 60, maximum: 3600 },
    stages: { type: 'array', minItems: 1, maxItems: 6, items: sessionScoreStageOutputSchema },
    preview: {
      type: 'object',
      properties: {
  ...
  required: ['capabilityId', 'version', 'correlationId', 'status', 'engine', 'durationSec', 'stages', 'preview', 'boundaries'], additionalProperties: false
};
...
const sessionScoreRequest = {
  ...
  properties: {
    ...
    score: { type: 'object', properties: { durationSec: { type: 'integer', minimum: 60, maximum: 3600 }, stages: { type: 'array', minItems: 1, maxItems: 6, items: sessionScoreStageSchema } }, required: ['durationSec', 'stages'], additionalProperties: false }
  },
  additionalProperties: false
};
    ...
    paths: {
      ...
      '/api/agent/session-score': {
        post: {
          ...
          description: 'Returns a technical-only, browser-local binaural score. Carrier is constant within each stage, beat changes linearly, and no persistence, rendering, or audio side effect occurs.',
