...
export const SESSION_SCORE_CAPABILITY_VERSION = '0.2.0';
export const SESSION_SCORE_PREVIEW_CAP_SEC = 120;
export const SESSION_SCORE_STATES = ['delta', 'theta', 'alpha', 'beta', 'gamma'];

const tenthStep = (value) => Math.abs(Number(value) * 10 - Math.round(Number(value) * 10)) < 1e-8;

const SessionScoreEntrainmentModesBaseSchema = z.object({
  binaural: z.boolean().default(true),
  monaural: z.boolean().default(false),
  isochronic: z.boolean().default(false)
}).strict();

export const SessionScoreEntrainmentModesSchema = SessionScoreEntrainmentModesBaseSchema.superRefine((modes, context) => {
  if (!Object.values(modes).some(Boolean)) {
    context.addIssue({ code: z.ZodIssueCode.custom, message: 'Enable at least one entrainment mode.' });
  }
});

export const SessionScoreBackgroundSchema = z.discriminatedUnion('type', [
  z.object({ type: z.literal('none') }).strict(),
  z.object({ type: z.literal('ocean'), mixDb: z.coerce.number().min(-60).max(-6).default(-24) }).strict(),
  z.object({
    type: z.literal('asset'),
    assetId: z.enum(AMBIENT_ASSET_IDS),
    mixDb: z.coerce.number().min(-60).max(-6).default(-25),
    crossfadeSec: z.coerce.number().min(0).max(10).default(2.5)
  }).strict()
]).default({ type: 'none' });

export const SessionScoreBreathGuideSchema = z.object({
  enabled: z.boolean().default(false),
  pattern: z.enum(['coherent-5.5', '4-7-8', 'box']).default('coherent-5.5'),
  bpm: z.coerce.number().min(2).max(12).default(5.5)
}).strict();

export const SessionScoreFadesSchema = z.object({
  inSec: z.coerce.number().min(0).max(60).default(8),
  outSec: z.coerce.number().min(0).max(60).default(12)
}).strict();

export const SessionScoreSoundSchema = z.object({
  entrainmentModes: SessionScoreEntrainmentModesSchema.default({ binaural: true, monaural: false, isochronic: false }),
  background: SessionScoreBackgroundSchema,
  breathGuide: SessionScoreBreathGuideSchema.default({ enabled: false, pattern: 'coherent-5.5', bpm: 5.5 }),
  fades: SessionScoreFadesSchema.default({ inSec: 8, outSec: 12 })
}).strict();

export const SessionScoreSoundPatchSchema = z.object({
  entrainmentModes: SessionScoreEntrainmentModesBaseSchema.partial().strict().optional(),
  background: SessionScoreBackgroundSchema.optional(),
  breathGuide: SessionScoreBreathGuideSchema.partial().strict().optional(),
  fades: SessionScoreFadesSchema.partial().strict().optional()
}).strict();
...
export const SessionScoreStageSchema = z.object({
  ...
  carrierHz: z.coerce.number().int().min(50).max(2000),
  beatHz: z.object({
    from: z.coerce.number().min(0.1).max(40).refine(tenthStep, 'Differential frequency must use 0.1 Hz increments.'),
    to: z.coerce.number().min(0.1).max(40).refine(tenthStep, 'Differential frequency must use 0.1 Hz increments.')
  }).strict(),
...
export const SessionScoreSchema = z.object({
  ...
  stages: z.array(SessionScoreStageSchema).min(1).max(12),
  sound: SessionScoreSoundSchema.default({
...
export const SessionScoreComposeInputSchema = z.object({
  ...
  sound: SessionScoreSoundSchema.optional()
...
export const SessionScoreRefineInputSchema = z.object({
  ...
  patch: z.object({
    ...
    carrierHz: z.coerce.number().int().min(50).max(2000).optional(),
    beatFromHz: z.coerce.number().min(0.1).max(40).refine(tenthStep, 'Differential frequency must use 0.1 Hz increments.').optional(),
    beatToHz: z.coerce.number().min(0.1).max(40).refine(tenthStep, 'Differential frequency must use 0.1 Hz increments.').optional(),
    volume: z.coerce.number().int().min(0).max(100).optional(),
    soundPatch: SessionScoreSoundPatchSchema.optional()
  }).strict().refine((patch) => Object.keys(patch).length > 0, 'At least one stage or sound field is required.')
}).strict();
...
export function composeSessionScore(input = {}) {
  ...
  if (parsed.score) {
    return publicScore(SessionScoreSchema.parse({
      ...parsed.score,
...
export function refineSessionScore(input = {}) {
  ...
  const sound = parsed.patch.soundPatch
    ? SessionScoreSoundSchema.parse({
      ...parsed.score.sound,
      ...parsed.patch.soundPatch,
      ...(parsed.patch.soundPatch.entrainmentModes ? {
        entrainmentModes: { ...parsed.score.sound.entrainmentModes, ...parsed.patch.soundPatch.entrainmentModes }
      } : {}),
      ...(parsed.patch.soundPatch.breathGuide ? {
        breathGuide: { ...parsed.score.sound.breathGuide, ...parsed.patch.soundPatch.breathGuide }
      } : {}),
      ...(parsed.patch.soundPatch.fades ? {
        fades: { ...parsed.score.sound.fades, ...parsed.patch.soundPatch.fades }
      } : {})
    })
    : parsed.score.sound;
  return publicScore(SessionScoreSchema.parse({ durationSec: parsed.score.durationSec, stages, sound }));
}

export function sessionScoreTechnicalExport(score) {
  const parsed = SessionScoreSchema.parse(score);
  return {
    format: 'cognistration-session-score-v2',
    engine: 'browser-signal-score',
    durationSec: parsed.durationSec,
