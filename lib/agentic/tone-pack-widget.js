...
export const TONE_PACK_CHECKOUT_WIDGET_RESOURCE_URI = 'ui://cognistration/tone-pack-checkout/v2.html';
      ...
      (function () {
        ...
        function showCheckout(data) {
          ...
          if (!link) {
            ...
            elements.status.parentNode.insertBefore(link, elements.status);
          }
        ...
        function callServerTool(name, args) {
          // MCP Apps is the portable contract. Keep window.openai as the
          // compatibility fallback for older ChatGPT hosts.
        ...
        function safeErrorMessage(error, fallback) {
          ...
          if (!message || message.length > 180 || /(?:sk_live|sk_test|password|secret|token)/i.test(message)) return fallback;
          return message;
        }
        ...
        function handleCheckout() {
          if (elements.checkout.dataset.mode === 'delivery') {
            ...
            callServerTool('get_tone_pack_delivery', { slug: selectedSlug, checkoutSessionId: checkoutSessionId })
              ...
              .catch(function (error) { setStatus(safeErrorMessage(error, 'Payment could not be verified yet. Complete checkout, then try again.'), 'error'); })
          ...
          callServerTool('create_tone_pack_checkout', { slug: selectedSlug, email: email, confirmed: true, idempotencyKey: idempotencyKey() })
            .then(function (result) { hydratePack(result); })
            .catch(function (error) { setStatus(safeErrorMessage(error, 'Checkout could not be created. Please try again.'), 'error'); })
