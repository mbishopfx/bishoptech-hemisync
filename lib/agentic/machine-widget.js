    ...
    <script>
      (function () {
        ...
        function publishModelContext() {
          ...
          var text = 'Current Cognistration machine state: ' + snapshot.targetState + ', carrier ' + snapshot.carrierHz + ' Hz, rhythm ' + snapshot.beatHz.toFixed(1) + ' Hz, volume ' + snapshot.volume + '%. Playback is ' + playback + '. The widget applies control changes live without pausing audio; state version ' + snapshot.stateVersion + '.';
        ...
        function applyOutput(value, options) {
          ...
          if (data.audioAction === 'start' && !outputOptions.skipAudioAction) audioActionFailed = startAudio() !== true;
          ...
          if (data.message && !audioActionFailed) setStatus(String(data.message), 'success');
        ...
        function startAudio() {
          if (playing && leftOscillator && rightOscillator) return true;
        ...
        elements.play.addEventListener('click', function () {
          if (playing) {
            ...
            var started = startAudio();
            callServerTool('start_machine_preview', { confirmed: true })
              .then(function (result) { handleServerResult(result, { skipAudioAction: started }); })
              .catch(function () {});
          }
        });
