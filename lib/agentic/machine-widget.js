  ...
  <body>
    ...
    <script>
      (function () {
        ...
        var sizeChangeTimer = null;
        var lastReportedSize = '';
        var bridgeInitialized = false;
        var bridgeInitPromise = null;
        var lastAction = 'ready';
        var startPromise = null;
        var startAttempt = 0;
        var watchedAudioContext = null;
        var hostGlobalsTimer = null;
        var lastHostOutputSignature = '';
        var lastHostStateSignature = '';
        ...
        function structuredContent(value) {
          ...
          if (parsed.structuredContent !== undefined) return structuredContent(parsed.structuredContent);
          // Some hosts expose the full tool response through
          // window.openai.toolResponseMetadata. That object can contain the
          // MCP result under either of these envelope keys.
          if (parsed.mcp_tool_result !== undefined) return structuredContent(parsed.mcp_tool_result);
          if (parsed.call_tool_result !== undefined) return structuredContent(parsed.call_tool_result);
          if (parsed.toolOutput !== undefined) return structuredContent(parsed.toolOutput);
          if (parsed.widgetOutput !== undefined) return structuredContent(parsed.widgetOutput);
          if (parsed.result !== undefined) return structuredContent(parsed.result);
          if (parsed.output !== undefined) return structuredContent(parsed.output);
          if (parsed.params !== undefined) return structuredContent(parsed.params);
          return parsed;
        }
        ...
        function hasMachineOutput(data) {
          if (!data || typeof data !== 'object') return false;
          return Boolean(
            data.capabilityId === 'cognistration-machine-generator' ||
            data.capabilityId === 'cognistration-machine-controls' ||
            data.resourceUri === 'ui://cognistration/machine-generator/v4.html' ||
            data.controls ||
            data.controlPatch ||
            data.adjustment ||
        ...
        function hostToolOutput(value) {
          var data = structuredContent(value);
          return hasMachineOutput(data) ? data : null;
        }
        ...
        function applyControls(next, options) {
          if (!next || typeof next !== 'object') return false;
          var controlOptions = options || {};
          var incomingVersion = Number(next.stateVersion);
          // A late host hydration event must never roll the visible machine
          // back over a newer control patch. Versioned controls are
          // authoritative; unversioned patches are fresh operations.
          if (Number.isFinite(incomingVersion) && incomingVersion < state.stateVersion) return false;
          var before = {
            targetState: state.targetState,
            carrierHz: state.carrierHz,
            beatHz: state.beatHz,
          ...
          if (Number.isFinite(incomingVersion)) {
            state.stateVersion = Math.max(1, incomingVersion);
          } else if (!controlOptions.preserveVersion) {
            state.stateVersion = Math.max(1, state.stateVersion + 1);
          }
          var changed = before.targetState !== state.targetState ||
            before.carrierHz !== state.carrierHz ||
            before.beatHz !== state.beatHz ||
            before.volume !== state.volume ||
            before.stateVersion !== state.stateVersion;
          updateOscillators();
          render();
          scheduleModelContextUpdate();
          return changed;
        ...
        function applyAdjustment(adjustment) {
          ...
          return applyControls(patch);
        }

        function applyHostOutput(value, options) {
          var data = hostToolOutput(value);
          if (!data) return null;
          var signature = valueSignature(data);
          if (signature && signature === lastHostOutputSignature) return data;
          lastHostOutputSignature = signature;
          applyOutput(data, options);
          return data;
        }
        ...
        function applyOutput(value, options) {
          ...
          if (!outputOptions.skipControlChanges) {
            if (data.controls) controlsChanged = applyControls(data.controls) || controlsChanged;
            if (data.controlPatch) controlsChanged = applyControls(data.controlPatch) || controlsChanged;
            if (data.calibration && data.calibration.controls) controlsChanged = applyControls(data.calibration.controls) || controlsChanged;
            if (data.adjustment && !data.controls && !data.controlPatch) controlsChanged = applyAdjustment(data.adjustment) || controlsChanged;
          }
          ...
          // Keep host widgetState authoritative after a model-issued control
          // result. This also gives hosts that hydrate globals asynchronously
          // a versioned snapshot to merge instead of an older reset.
          if (controlsChanged) persistState();
          render();
          scheduleModelContextUpdate();
        }
        ...
        function sendBridgeNotification(method, params) {
        ...
        function reportSize() {
          ...
          var signature = String(width) + 'x' + String(height);
          var openai = window.openai;
          if (openai && typeof openai.notifyIntrinsicHeight === 'function') {
            try { openai.notifyIntrinsicHeight(height); } catch (error) {}
          }
          if (!bridgeInitialized || signature === lastReportedSize) return;
          lastReportedSize = signature;
          sendBridgeNotification('ui/notifications/size-changed', { width: width, height: height });
        }

        function scheduleSizeReport() {
          if (sizeChangeTimer !== null) window.clearTimeout(sizeChangeTimer);
          sizeChangeTimer = window.setTimeout(function () {
        ...
        function watchSizeChanges() {
          ...
          var observer = new window.ResizeObserver(scheduleSizeReport);
          if (document.documentElement) observer.observe(document.documentElement);
          if (document.body) observer.observe(document.body);
        }

        // MCP Apps hosts hold tool notifications until the View completes the
        // initialize handshake. Without this, a model-issued control can be
        // acknowledged in chat while the already-mounted machine never sees
        // the returned controlPatch.
        function initializeBridge() {
          if (window.parent === window || bridgeInitPromise) return bridgeInitPromise || Promise.resolve(null);
          bridgeInitPromise = requestBridge('ui/initialize', {
            appInfo: { name: 'cognistration-machine-generator', version: '0.2.0' },
            appCapabilities: {
            ...
            lastReportedSize = '';
            sendBridgeNotification('ui/notifications/initialized', {});
            scheduleModelContextUpdate();
            scheduleSizeReport();
            return result;
          }).catch(function () {
            // Keep the widget usable in older hosts that only implement the
            // pre-MCP Apps postMessage bridge.
            bridgeInitialized = true;
            lastReportedSize = '';
            // A few hosts accept notifications but do not answer the
            // initialize request. Send the lifecycle acknowledgement anyway so
            // they can release any queued model result for this view.
            sendBridgeNotification('ui/notifications/initialized', {});
            scheduleSizeReport();
            return null;
          });
        ...
        function applyHostWidgetState(next) {
          if (!next || typeof next !== 'object') return;
          var signature = valueSignature(next);
          if (signature && signature === lastHostStateSignature) return;
          lastHostStateSignature = signature;
          applyControls(next, { preserveVersion: true });
        }

        function applyHostGlobals(globals) {
          if (!globals || typeof globals !== 'object') return;
          if (globals.widgetState && typeof globals.widgetState === 'object') applyHostWidgetState(globals.widgetState);
          if (globals.toolInput && globals.toolInput.intention && !elements.intention.value) {
            elements.intention.value = String(globals.toolInput.intention).slice(0, 240);
          }
          // Prefer the full response metadata when a host exposes it. Older
          // compatibility hosts only expose toolOutput, so keep that path.
          var metadataOutput = globals.toolResponseMetadata !== undefined
            ? hostToolOutput(globals.toolResponseMetadata)
            : null;
          if (metadataOutput) applyHostOutput(metadataOutput);
          else if (globals.toolOutput !== undefined) applyHostOutput(globals.toolOutput);
          else if (globals.widgetOutput !== undefined) applyHostOutput(globals.widgetOutput);
        }

        // Some Codex/ChatGPT surfaces hydrate window.openai after the iframe
        // mounts but do not emit openai:set_globals for every model tool call.
        // Polling the documented globals is a compatibility fallback for that
        // host shape; signatures keep it cheap and prevent duplicate patches.
        function pollHostGlobals() {
          var openai = window.openai;
          if (openai) {
            applyHostWidgetState(openai.widgetState);
            var metadataOutput = hostToolOutput(openai.toolResponseMetadata);
            if (metadataOutput) applyHostOutput(metadataOutput);
            else {
              var toolOutput = openai.toolOutput !== undefined ? openai.toolOutput : openai.widgetOutput;
              if (toolOutput !== undefined) applyHostOutput(toolOutput);
            }
          }
          hostGlobalsTimer = window.setTimeout(pollHostGlobals, 350);
        }

        function watchHostGlobals() {
          if (hostGlobalsTimer !== null) return;
          pollHostGlobals();
        }

        function handleServerResult(value, options) {
          return applyHostOutput(value, options) || structuredContent(value);
        }
        ...
        window.addEventListener('message', function (event) {
          // The MCP Apps bridge normally runs in an iframe. Codex can also
          // present a ui:// resource in a top-level app panel, where the
          // parent is the page itself and host messages use window as source.
          var validSource = window.parent === window
            ? (!event.source || event.source === window)
            : event.source === window.parent;
          if (!validSource) return;
          var message = typeof event.data === 'string' ? safeJson(event.data) : event.data;
          if (!message) return;
          ...
          if (message.jsonrpc === '2.0' && message.method === 'ui/notifications/tool-result') {
            applyHostOutput(message.params);
            return;
          }
          // A few hosts still use the older typed envelope. Accept it as a
          // compatibility path while keeping the standard JSON-RPC shape
          // authoritative.
          if (message.type === 'ui/notifications/tool-result' || message.type === 'openai/widget-output') {
            applyHostOutput(message.result || message.output || message);
            return;
          }
          if (message.type === 'openai:set_globals' || message.method === 'openai/set_globals') {
            applyHostGlobals(message.globals || message.params || message.detail);
            return;
          }
          if (message.jsonrpc === '2.0' && message.method === 'ui/notifications/tool-input') {
            var params = message.params || {};
            var input = params.arguments || params.input || params;
            if (input && input.intention && !elements.intention.value) elements.intention.value = String(input.intention).slice(0, 240);
          }
        }, { passive: true });

        // ChatGPT's compatibility bridge can publish the same tool result via
        // openai:set_globals. Keep this listener as a fallback for hosts that
        // hydrate window.openai after the View has mounted.
        window.addEventListener('openai:set_globals', function (event) {
          applyHostGlobals(event && event.detail && event.detail.globals);
        }, { passive: true });

        initializeBridge();
        watchSizeChanges();
        var savedState = window.openai && window.openai.widgetState;
        if (savedState && typeof savedState === 'object') applyHostWidgetState(savedState);
        ...
        applyHostOutput(window.openai && window.openai.toolOutput);
        applyHostOutput(window.openai && window.openai.toolResponseMetadata);
