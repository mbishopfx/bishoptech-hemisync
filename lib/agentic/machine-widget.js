...
<html lang="en">
  <head>
    ...
    <style>
      ...
      * { box-sizing: border-box; }
      ...
      .signal-visual { position: relative; min-height: 190px; overflow: hidden; border: 1px solid rgba(226,243,235,.09); border-radius: 16px; background: rgba(2, 12, 10, .48); padding: 22px 16px; }
      .signal-visual::after { content: ""; position: absolute; inset: 0; background: radial-gradient(circle at 50% 52%, rgba(199,232,216,.13), transparent 58%); pointer-events: none; }
      .wave { position: absolute; left: 5%; width: 90%; height: 1px; transform-origin: center; opacity: .75; }
      .wave::before { content: ""; position: absolute; inset: -14px 0; background: repeating-linear-gradient(90deg, transparent 0 4px, currentColor 4px 6px, transparent 6px 13px); mask-image: radial-gradient(ellipse at center, black 0 55%, transparent 75%); -webkit-mask-image: radial-gradient(ellipse at center, black 0 55%, transparent 75%); }
      .wave-left { top: 38%; color: var(--mint); animation: wave-left 4s linear infinite; }
      .wave-right { top: 58%; color: var(--peach); animation: wave-right 3.5s linear infinite reverse; }
      .wave-beat { top: 80%; color: var(--lavender); opacity: .85; animation: wave-beat 5.2s ease-in-out infinite; }
      @keyframes wave-left { to { transform: translateX(12px) scaleX(1.05); } }
      @keyframes wave-right { to { transform: translateX(-14px) scaleX(.94); } }
      @keyframes wave-beat { 50% { transform: scaleX(.7); opacity: .32; } }
  ...
  <body>
    <main class="machine-shell" data-machine-widget>
      ...
      (function () {
        ...
        function startAudio() {
          ...
          try {
            ...
            setStatus('The browser could not start local audio. Try pressing play again.', 'error');
          }
        }
        ...
        function handleServerResult(value) {
          var data = structuredContent(value);
          if (data && data.error) {
            setStatus(String(data.error.safeMessage || 'That request could not be completed.'), 'error');
          }
          applyOutput(value);
          return data;
        }
        ...
        elements.play.addEventListener('click', function () { if (playing) stopAudio(); else startAudio(); });
        elements.packs.addEventListener('click', browsePacks);
        elements.gamma.addEventListener('click', function () { applyStatePreset('gamma', 39.5); applyControls({ carrierHz: 246 }); setStatus('Gamma is set with a 246 Hz carrier.', 'success'); persistState(); });
        elements.smaller.addEventListener('click', function () { applyControls({ carrierHz: Math.max(100, state.carrierHz - 24) }); setStatus('Carrier lowered to ' + String(state.carrierHz) + ' Hz.', 'success'); persistState(); });
