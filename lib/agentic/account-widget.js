...
// Publish a fresh URI whenever the embedded form changes so ChatGPT cannot
// keep serving a cached widget with an older submission path.
export const ACCOUNT_SIGNUP_WIDGET_RESOURCE_URI = 'ui://cognistration/account-signup/v3.html';
export const ACCOUNT_SIGNUP_PREVIOUS_WIDGET_RESOURCE_URI = 'ui://cognistration/account-signup/v2.html';
// Keep previous URIs readable for conversations that cached an earlier tool
// descriptor before the widget resource was refreshed.
export const ACCOUNT_SIGNUP_LEGACY_WIDGET_RESOURCE_URI = 'ui://cognistration/account-signup/v1.html';
export const ACCOUNT_SIGNUP_COMPATIBILITY_RESOURCE_URIS = [
  ...
  <head>
    ...
    <style>
      ...
      * { box-sizing: border-box; }
      ...
      .fallback { margin-top: 14px; border: 1px solid rgba(242, 199, 174, .2); border-radius: 14px; background: rgba(242, 199, 174, .06); padding: 12px; }
      .fallback p { margin: 0; color: var(--muted); font-size: 11px; line-height: 1.5; }
      .fallback-link { display: inline-flex; margin-top: 10px; border-radius: 999px; background: rgba(199, 232, 216, .13); padding: 9px 12px; color: var(--mint); font-size: 11px; font-weight: 700; text-decoration: none; transition: background .2s ease, transform .2s ease; }
      .fallback-link:hover { background: rgba(199, 232, 216, .22); transform: translateY(-1px); }
  ...
  <body>
    <main class="account-shell" data-account-widget>
      ...
      <section class="form-panel" aria-labelledby="form-title">
        ...
        <form id="account-form" novalidate>
          ...
          <div class="fallback" id="account-fallback" hidden>
            <p>The embedded app could not reach the first-party signup service. Your credentials were not submitted through MCP.</p>
            <a class="fallback-link" id="account-fallback-link" href="https://cognistration.com/signup?source=mcp" target="_blank" rel="noreferrer">Open secure signup page</a>
          </div>
        </form>
    ...
    <script>
      (function () {
        ...
        var fallbackLink = document.getElementById('account-fallback-link');
        ...
        fallbackLink.addEventListener('click', function (event) {
          ...
          openExternal('https://cognistration.com/signup?source=mcp').catch(function () {
            setStatus('Open the secure signup page in a new tab to continue.', 'error');
          });
        });
        ...
        form.addEventListener('submit', async function (event) {
          ...
          try {
            var response = await fetch('https://cognistration.com/api/agent/account/signup', {
              ...
              credentials: 'omit',
              cache: 'no-store',
              // The endpoint parses the raw body as JSON. text/plain keeps this
              // credential submission CORS-simple inside sandboxed MCP Apps,
              // avoiding a host-dependent application/json preflight.
              headers: { 'Content-Type': 'text/plain' },
              body: JSON.stringify({ username: username, email: email, password: password })
            });
            var data = await response.json().catch(function () { return {}; });
            if (!response.ok) throw new Error(data.error || 'The account service returned HTTP ' + String(response.status) + '.');
            form.hidden = true;
            ...
            var isNetworkFailure = error && (error.name === 'TypeError' || error.message === 'Failed to fetch' || error.message === 'Load failed');
            if (isNetworkFailure) {
              fallback.hidden = false;
              setStatus('The embedded signup service could not be reached. Try the secure signup page below.', 'error');
            } else {
