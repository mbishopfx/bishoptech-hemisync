...
function rankedPublicTonePacks(input = {}) {
...
export function searchPublicTonePacks(input = {}) {
  const parsed = TonePackSearchInputSchema.parse(input);
  return rankedPublicTonePacks(parsed).slice(0, parsed.limit);
}
...
function decodeCursor(cursor) {
  ...
  if (parsed?.v !== 1 || !Number.isInteger(parsed.index) || parsed.index < 0 || parsed.index > PUBLIC_TONE_PACK_CATALOG.length) throw new Error('Invalid tone-pack cursor.');
  return parsed.index;
}

export function searchPublicTonePacksPage(input = {}) {
  const { cursor, ...searchInput } = input;
  const parsed = TonePackSearchInputSchema.parse(searchInput);
  const ranked = rankedPublicTonePacks(parsed);
  const start = decodeCursor(cursor);
  const packs = ranked.slice(start, start + parsed.limit);
  const nextCursor = start + packs.length < ranked.length ? encodeCursor(start + packs.length) : null;
