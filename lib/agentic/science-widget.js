...
<html lang="en">
  <head>
    ...
    <style>
      :root {
        ...
        --line: rgba(182, 221, 204, 0.11);
        --glass-edge: rgba(182, 221, 204, 0.16);
        --glass-highlight: rgba(242, 246, 242, 0.08);
        --glass: linear-gradient(135deg, rgba(182, 221, 204, 0.1), rgba(255, 255, 255, 0.025)), rgba(18, 36, 31, 0.72);
        --mint: #b6ddcc;
        --deep: #10221d;
      }
      ...
      * { box-sizing: border-box; }
      ...
      .slide-card {
        ...
        box-shadow: inset 0 1px 0 var(--glass-highlight), inset 0 -18px 40px rgba(7, 22, 18, 0.12), 0 26px 80px rgba(0, 0, 0, 0.24);
      ...
      .print-button,
      .nav-button {
        border: 1px solid rgba(182, 221, 204, 0.18);
        border-radius: 999px;
        color: var(--muted);
        background: linear-gradient(135deg, rgba(182, 221, 204, 0.12), rgba(255, 255, 255, 0.025)), rgba(16, 34, 29, 0.42);
        padding: 9px 13px;
        transition: border-color 160ms ease, background-color 160ms ease, color 160ms ease, transform 160ms ease;
      }

      .print-button:hover,
      .nav-button:hover { border-color: rgba(182, 221, 204, 0.42); background: linear-gradient(135deg, rgba(182, 221, 204, 0.2), rgba(255, 255, 255, 0.04)), rgba(16, 34, 29, 0.52); color: var(--ink); transform: translateY(-1px); }
      .print-button:active,
      ...
      .pdf-direct-link {
        ...
        text-decoration-color: rgba(182, 221, 204, 0.35);
        text-underline-offset: 4px;
      }

      .pdf-direct-link:hover { color: var(--ink); }
      ...
      .fact-list {
        ...
        gap: 0;
        margin: 26px 0 0;
        border-top: 1px solid rgba(182, 221, 204, 0.09);
      }
      ...
      .band-row {
        ...
        gap: 14px;
        padding: 12px 0;
        border-bottom: 1px solid rgba(182, 221, 204, 0.09);
      }
      ...
      .source-list a {
        ...
        font-size: 12px;
        text-decoration: underline;
        text-decoration-color: rgba(182, 221, 204, 0.22);
        text-underline-offset: 4px;
      }
  ...
  <body>
    <main class="science-shell" aria-labelledby="science-title">
      ...
      <div id="ocean-status" class="sr-only" role="status" aria-live="polite">Starting WebGPU</div>
      ...
      <div class="science-content">
        <div class="science-topline">
          ...
          <div class="science-actions">
            ...
            <a class="pdf-direct-link" id="pdf-direct-link" href="#" target="_blank" rel="noreferrer" hidden>Open PDF directly</a>
          </div>
        </div>
    ...
    <script>
      (function () {
        ...
        var elements = {
          ...
          pdfDirectLink: document.getElementById('pdf-direct-link')
        };
        ...
        function renderPdfLink() {
          if (elements.pdfDirectLink) elements.pdfDirectLink.href = buildPdfUrl();
        }

        function buildPdfUrl() {
          var params = new URLSearchParams({
            targetState: String(controls.targetState || 'theta'),
            carrierHz: String(Math.round(Number(controls.carrierHz) || 200)),
            beatHz: String(Number(controls.beatHz || 6).toFixed(1)),
            volume: String(Math.round(Number(controls.volume) || 72))
          });
          if (tone && tone.id) params.set('toneId', String(tone.id));
          if (window.__cognistrationOceanProfile && window.__cognistrationOceanProfile.seed !== undefined) {
            params.set('oceanSeed', String(window.__cognistrationOceanProfile.seed));
          }
          return 'https://cognistration.com/api/science-guide/pdf?' + params.toString();
        }

        function downloadPdf() {
          elements.print.disabled = true;
          elements.print.textContent = 'Preparing PDF...';
          var exportUrl = buildPdfUrl();
          if (elements.pdfDirectLink) {
            elements.pdfDirectLink.href = exportUrl;
            elements.pdfDirectLink.hidden = false;
          }
          try {
            var absoluteUrl = new URL(exportUrl, window.location.href).toString();
            var openExternal = window.openai && window.openai.openExternal;
            if (typeof openExternal === 'function') {
              var result = openExternal({ href: absoluteUrl, redirectUrl: false });
              if (result && typeof result.catch === 'function') {
                result.catch(function () { setPdfStatus('The host could not open the PDF. Use the direct link below.'); });
              }
              setPdfStatus('PDF download started.');
              return;
            }

            // Keep the navigation inside the user click. An awaited blob URL
            // can be blocked as an unsolicited download in a sandboxed MCP App.
            var link = document.createElement('a');
            ...
            link.click();
            link.remove();
            setPdfStatus('PDF export opened. If it did not download, use the direct link below.');
          } catch (error) {
            console.error('Science guide PDF export failed:', error);
            setPdfStatus('PDF export was blocked. Use the direct link below or your browser print command.');
          } finally {
