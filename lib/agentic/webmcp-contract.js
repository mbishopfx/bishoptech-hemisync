...
const sessionScoreStageSchema = {
  ...
  properties: {
    ...
    beatHz: {
      type: 'object',
      properties: {
        from: { type: 'number', minimum: 0.5, maximum: 40, multipleOf: 0.5 },
        to: { type: 'number', minimum: 0.5, maximum: 40, multipleOf: 0.5 }
      },
      required: ['from', 'to'],
      additionalProperties: false
    },
...
const sessionScoreSchema = {
  ...
  properties: {
    ...
    stages: { type: 'array', minItems: 1, maxItems: 6, items: sessionScoreStageSchema }
  },
...
export const WEBMCP_TOOL_DEFINITIONS = [
  {
    ...
    description: 'Compose or validate a browser-local score with one to six stages. No save, render.',
    inputSchema: {
      type: 'object',
      properties: {
  ...
  },
  {
    ...
    description: 'Refine one score stage; duration and order stay fixed.',
    inputSchema: {
      type: 'object',
      properties: {
        ...
        state: { type: 'string', enum: ['delta', 'theta', 'alpha', 'beta', 'gamma'] },
        carrierHz: { type: 'integer', minimum: 100, maximum: 400 },
        beatFromHz: { type: 'number', minimum: 0.5, maximum: 40, multipleOf: 0.5 },
        beatToHz: { type: 'number', minimum: 0.5, maximum: 40, multipleOf: 0.5 },
        volume: { type: 'integer', minimum: 0, maximum: 100 }
      },
