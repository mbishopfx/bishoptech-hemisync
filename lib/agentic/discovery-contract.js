...
export function authorizationServerMetadata(origin = discoveryOrigin()) {
  ...
  if (upstream) {
    return {
      // Cognistration delegates user authentication to the configured Supabase
      // OIDC issuer. This alias intentionally reports that issuer so clients
      // can validate the issuer against Protected Resource Metadata.
      ...upstream,
      authorization_server_status: 'provider_delegated',
      ...
      implementationNote: 'Supabase OIDC is the configured provider for user-controlled sign-in and bearer sessions. Cognistration does not implement the auth.md agentic-registration ceremony, dynamic client registration, or an agent-minted identity assertion. Public discovery and preview routes are anonymous; agents must never request or transmit a password, service key, card, or payment credential.'
    };
  }
...
This document includes an explicit \`agent_auth\` marker so compatible clients can discover the boundary without guessing. Its status is intentionally \`discovery_only\`: Cognistration does not implement the auth.md agentic-registration ceremony, dynamic client registration, ID-JAG exchange, or agent-minted identity assertions. The supported path is a person-controlled first-party sign-in followed by a user-scoped bearer session.
...
\`\`\`json
{
  "agent_auth": {
    ...
    "claim_endpoint": "/agent/claim",
    "events_endpoint": "/agent/events",
    "revocation_uri": "/auth/revoke",
    "identity_types_supported": [],
    "credential_types_supported": ["bearer"],
    "identity_assertion": {
      "assertion_types_supported": []
...
The empty identity and assertion arrays are deliberate. They tell an agent not to POST a password, email-only registration, ID-JAG, claim token, or payment credential to these routes.
...
When Supabase is configured, authorization-server metadata is provider-delegated: it reports the Supabase OIDC issuer and its user-controlled PKCE endpoints. Cognistration’s \`/auth/authorize\`, \`/auth/token\`, and \`/auth/revoke\` paths remain deliberate fail-closed stubs, not token issuers. Without a configured provider, the metadata falls back to \`discovery_only\` and does not advertise a token flow. Public calls can remain anonymous; private access is completed through the first-party sign-in flow. Never exchange a password, Supabase service-role key, Stripe key, or payment credential at an agent endpoint.
