    ...
    skills: [
      {
        ...
        protocol: 'MCP',
        source: `${normalizeOrigin(origin)}/skills/cognistration-agentic-routing/SKILL.md`,
        endpoint: links.mcp,
      ...
      },
      {
        ...
        protocol: 'MCP',
        source: `${normalizeOrigin(origin)}/skills/cognistration-tone-orchestration/SKILL.md`,
        endpoint: links.mcp,
      ...
      },
      {
        ...
        protocol: 'MCP Apps',
        source: `${normalizeOrigin(origin)}/skills/cognistration-account-safety/SKILL.md`,
        endpoint: links.mcp,
      ...
      },
      {
        ...
        protocol: 'MCP',
        source: `${normalizeOrigin(origin)}/skills/cognistration-agent-evaluation/SKILL.md`,
      ...
      },
      {
        ...
        protocol: 'MCP Apps',
        source: `${normalizeOrigin(origin)}/skills/cognistration-feedback/SKILL.md`,
        endpoint: links.mcp,
...
export function protectedResourceMetadata(origin = discoveryOrigin()) {
  const links = discoveryLinks(origin);
  return {
    resource: `${normalizeOrigin(origin)}/api/`,
    authorization_servers: [links.authorizationServer],
    scopes_supported: [...PUBLIC_AGENT_SCOPES],
...
export function authorizationServerMetadata(origin = discoveryOrigin()) {
  const links = discoveryLinks(origin);
  const enabled = String(process.env.COGNISTRATION_OAUTH_ENABLED || '').toLowerCase() === 'true';
  return {
    issuer: normalizeOrigin(origin),
    authorization_server_status: enabled ? 'enabled' : 'discovery_only',
    authorization_endpoint: `${normalizeOrigin(origin)}/auth/authorize`,
    token_endpoint: `${normalizeOrigin(origin)}/auth/token`,
    revocation_endpoint: `${normalizeOrigin(origin)}/auth/revoke`,
    response_types_supported: enabled ? ['code'] : [],
    grant_types_supported: enabled ? ['authorization_code', 'refresh_token'] : [],
    code_challenge_methods_supported: enabled ? ['S256'] : [],
    token_endpoint_auth_methods_supported: ['none'],
    scopes_supported: [...PUBLIC_AGENT_SCOPES],
    service_documentation: links.auth,
    agent_auth: {
      skill: links.auth,
      identity_endpoint: `${normalizeOrigin(origin)}/agent/identity`,
      claim_endpoint: `${normalizeOrigin(origin)}/agent/claim`,
      events_endpoint: `${normalizeOrigin(origin)}/agent/events`,
      identity_types_supported: ['anonymous', 'identity_assertion', 'service_auth'],
      identity_assertion: {
        assertion_types_supported: ['urn:ietf:params:oauth:token-type:id-jag']
      }
    },
    implementationNote: enabled
      ? 'Public discovery and preview routes are anonymous. Private member access uses the enabled PKCE authorization-code flow; agents must use the user-controlled sign-in flow and never request or transmit a password.'
      : 'OAuth endpoints are published for discovery but are not enabled in this deployment. Public discovery and preview routes are anonymous. Private member access uses the first-party bearer session issued by Cognistration’s configured identity provider; agents must use the user-controlled sign-in flow and never request or transmit a password.'
  };
}
...
Use anonymous access for \`public.read\` and \`public.session\` discovery. A signed-in member session is required for \`member.read\` and \`member.write\`. Checkout operations are scoped separately as \`public.checkout\` and still require explicit confirmation. The declared scopes are least-privilege labels, not permission to bypass a form or consent step.

## Register

The public product does not issue API keys from chat. If a client needs a private member session, the person must use the first-party [sign-in page](/login) and the configured identity provider. Do not invent a client secret or use a service key in a browser.

## Claim

The advertised \`agent_auth\` endpoints are safe discovery hooks for compatible agent identity ceremonies: [identity](/agent/identity), [claim](/agent/claim), and [events](/agent/events). They do not accept passwords or payment data. A client must follow the response instructions and stop when a user step is required.

## Exchange

The authorization-server metadata reports whether Cognistration’s optional PKCE authorization-code flow is enabled. In the default discovery-only deployment, the \`/auth/authorize\`, \`/auth/token\`, and \`/auth/revoke\` paths are deliberate fail-closed stubs, not token issuers. Public calls can remain anonymous; private access is completed through the first-party sign-in flow. Never exchange a password, Supabase service-role key, Stripe key, or payment credential at an agent endpoint.
...
export function pageMarkdown(pathname, origin = discoveryOrigin()) {
  const base = normalizeOrigin(origin);
  if (pathname === '/' || pathname === '/index.md') return ROOT_MARKDOWN.replaceAll('](/', `](${base}/`);
  if (pathname === '/pricing' || pathname === '/pricing.md') return PRICING_MARKDOWN.replaceAll('](/', `](${base}/`);
  if (pathname === '/docs' || pathname === '/docs.md') return DOCS_MARKDOWN.replaceAll('](/', `](${base}/`);
  if (pathname === '/auth.md') return AUTH_MARKDOWN.replaceAll('](/', `](${base}/`);
  const pageContent = {
  ...
  if (pageContent[pathname] || pageContent[pathname.replace(/\.md$/, '')]) return pageContent[pathname] || pageContent[pathname.replace(/\.md$/, '')];
  return `# Cognistration ${pathname}\n\nRead the [Cognistration developer index](${base}/docs.md) or [agent instructions](${base}/agent-instructions.md) for the machine-readable public surface.\n`;
}
...
export function scopedMarkdown(section, origin = discoveryOrigin()) {
  ...
  return sections[section] || sections.docs;
}
