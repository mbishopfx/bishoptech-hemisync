const DEFAULT_ALLOWED_HEADERS = 'Authorization, Content-Type, X-User-Id, MCP-Protocol-Version, Mcp-Method, Mcp-Name, Idempotency-Key, Payment-Authorization';
const DEFAULT_ALLOWED_METHODS = 'GET, POST, OPTIONS';
const DEFAULT_EXPOSED_HEADERS = 'Accept-Ranges, Content-Disposition, Content-Length, Content-Range, MCP-Protocol-Version, RateLimit-Limit, RateLimit-Remaining, RateLimit-Reset, RateLimit-Policy, Retry-After';
...
// OpenAI may use a versioned subdomain for an opaque MCP Apps sandbox. Keep
// the allowance restricted to OpenAI-owned HTTPS sandbox origins rather than
// accepting arbitrary origins for credential-bearing first-party forms.
const openAiSandboxOrigin = /^https:\/\/[a-z0-9-]+\.oaiusercontent\.com$/i;
...
export function resolveAllowedOrigin(origin) {
  ...
  if (openAiSandboxOrigin.test(normalizeOrigin(origin))) {
    return origin;
  }
