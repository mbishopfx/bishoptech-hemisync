...
const GuidanceDensitySchema = z.enum(['light', 'medium', 'high']);
...
const StageBlueprintSchema = z.object({
  ...
  guidanceDensity: GuidanceDensitySchema.optional(),
...
export const CombinedAudioInputSchema = GenerateAudioInputSchema.extend({
  ...
  guidanceMode: z.enum(['ai', 'preset', 'hybrid']).default('hybrid'),
  ducking: z
    .object({
      enabled: z.boolean().optional(),
      bedPercentWhileTalking: z.number().min(0.2).max(1).optional(),
      attackMs: z.number().min(0).max(1000).optional(),
      releaseMs: z.number().min(0).max(2000).optional()
    })
    .partial()
    .optional(),
  tts: z
    .object({
      ...
      mixDb: z.number().min(-36).max(-6).optional(),
