...
export const JournalInputSchema = z.object({
  ...
  mood: z.enum(['clear', 'steady', 'overloaded', 'tender', 'energized', 'restless']).optional(),
  energy: z.coerce.number().int().min(1).max(5).optional(),
  focusArea: z.string().trim().max(80).optional(),
  tags: z.array(z.string().trim().min(1).max(32)).max(6).optional(),
...
export const JournalPatchSchema = z.object({
  id: z.string().uuid(),
  title: z.string().trim().max(120).optional(),
  mood: z.enum(['clear', 'steady', 'overloaded', 'tender', 'energized', 'restless']).nullable().optional(),
  energy: z.coerce.number().int().min(1).max(5).optional(),
...
export const GenerateAudioInputSchema = z.object({
  ...
  focusLevel: z.string().optional(),
  lengthSec: z.number().int().min(120).max(1800).default(900),
