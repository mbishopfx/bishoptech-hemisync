...
export function ucpProfile(origin = siteOrigin()) {
  ...
  const officialAp2 = officialAp2CapabilityEnabled();
    ...
    ucp: {
      ...
      capabilities: {
        ...
        ...(officialAp2 ? {
          [AP2_UCP_CAPABILITY]: [{
            version: UCP_VERSION,
            spec: AP2_UCP_SPEC_URL,
            schema: AP2_UCP_SCHEMA_URL,
...
function publicPaymentInstrument(instrument) {
  if (!instrument || typeof instrument !== 'object' || Array.isArray(instrument)) return null;
  const safe = {};
  for (const field of ['type', 'handler_id', 'provider']) {
    if (typeof instrument[field] === 'string' && instrument[field].length <= 120) safe[field] = instrument[field];
  }
  return Object.keys(safe).length ? safe : null;
}

export function publicUcpPayment(payment = {}) {
  const instruments = Array.isArray(payment.instruments)
    ? payment.instruments.map(publicPaymentInstrument).filter(Boolean)
    : [];
  return {
    ...(Array.isArray(payment.instruments) ? { instruments } : {}),
    ...(payment.handler_id ? { handler_id: String(payment.handler_id).slice(0, 120) } : {}),
    ...(payment.provider ? { provider: String(payment.provider).slice(0, 120) } : {}),
    ...(payment.order && typeof payment.order === 'object' ? { order: payment.order } : {})
  };
}
...
function checkoutResource(row, origin = siteOrigin()) {
  ...
  const publicPayment = publicUcpPayment(payment);
  const resource = {
    ucp: {
      version: UCP_VERSION,
      capabilities: {
        ...
        ...(officialAp2 ? { [AP2_UCP_CAPABILITY]: [{ version: UCP_VERSION }] } : {})
      },
      payment_handlers: paymentHandlers(origin)
    },
    id: row.id,
  ...
  if (officialAp2) {
    const ap2 = payment.ap2 && typeof payment.ap2 === 'object' ? { ...payment.ap2 } : {};
    const merchantAuthorization = createMerchantAuthorization({ checkout: resource });
    if (merchantAuthorization) ap2.merchant_authorization = merchantAuthorization;
    if (Object.keys(ap2).length) resource.ap2 = ap2;
  }
  return resource;
}
...
async function persistUcpOrder({ admin, checkout, payment, fulfillment, status = 'fulfilled' }) {
  ...
  const values = {
    ...
    payment: {
      ...existingPayment,
      ...nextPayment,
      ...(existingPayment.ap2 || nextPayment.ap2 ? {
        ap2: { ...(existingPayment.ap2 || {}), ...(nextPayment.ap2 || {}) }
      } : {})
    },
    fulfillment,
    updated_at: now
...
function extractOfficialCheckoutMandate(body) {
  return body?.ap2?.checkout_mandate
    || body?.checkout?.ap2?.checkout_mandate
    || null;
}
...
async function completeWithSpt({ checkout, payment, admin, origin, mandateReceipt }) {
  ...
  const packSlug = checkout.line_items?.[0]?.item?.id;
  const existingOrder = mandateReceipt?.official ? await getOrderByCheckout(admin, checkout.id) : null;
  const paymentReceipt = mandateReceipt?.official
    ? (existingOrder?.payment?.ap2?.payment_receipt || createOfficialAp2PaymentReceipt({
      token: mandateReceipt.paymentMandateToken,
      paymentId: paymentIntent.id,
      pspConfirmationId: paymentIntent.id
    }))
    : null;
  ...
  const persisted = await ensureOrderDelivery({
    ...
    ...(await persistUcpOrder({
      ...
      payment: {
        ...
        ...(paymentReceipt ? { ap2: { payment_receipt: paymentReceipt } } : {}),
  ...
  let ap2Receipts = null;
  if (mandateReceipt?.official) {
    const storedReceipts = order.payment?.ap2 && typeof order.payment.ap2 === 'object' ? order.payment.ap2 : {};
    ap2Receipts = {
      checkout_receipt: storedReceipts.checkout_receipt || createOfficialAp2CheckoutReceipt({
        token: mandateReceipt.checkoutMandateToken,
        orderId: order.id
      }),
      payment_receipt: storedReceipts.payment_receipt || paymentReceipt
    };
    const { data: orderWithReceipts, error: receiptError } = await admin.from('commerce_orders').update({
      payment: { ...(order.payment || {}), ap2: ap2Receipts },
      updated_at: new Date().toISOString()
  ...
  const confirmation = {
    ...buildOrderConfirmation(order, origin),
    ...(ap2Receipts ? { ap2: ap2Receipts } : {})
  };
  const { data: updated, error: updateError } = await admin.from('commerce_checkouts').update({
    status: 'completed',
...
export async function completeUcpCheckout({ id, body, idempotencyKey, origin = siteOrigin(), supabase } = {}) {
  ...
  const officialCheckoutMandate = extractOfficialCheckoutMandate(body);
  const officialPaymentMandate = extractOfficialPaymentMandate(payment);
  if (officialPaymentMandate && !officialCheckoutMandate) {
    throw commerceError('AP2_CHECKOUT_MANDATE_REQUIRED', 'An official AP2 payment mandate must be paired with its checkout mandate.', 400);
  }
  const officialMandateReceipt = officialCheckoutMandate
    ? verifyOfficialAp2CheckoutMandate({
      token: officialCheckoutMandate,
      checkout: checkoutResource(checkout, origin)
    })
    : null;
  if (officialCheckoutMandate && !officialPaymentMandate) {
    throw commerceError('AP2_PAYMENT_MANDATE_REQUIRED', 'An official AP2 checkout mandate must be paired with its payment mandate.', 400);
  }
  const officialPaymentReceipt = officialPaymentMandate
    ? verifyOfficialAp2PaymentMandate({
      token: officialPaymentMandate,
      checkout: checkoutResource(checkout, origin),
      checkoutMandate: officialMandateReceipt
    })
    : null;
  const mandate = payment.mandate || payment.autonomous_mandate;
  if (mandate && officialCheckoutMandate) {
    throw commerceError('AP2_MANDATE_CONFLICT', 'Use one AP2 mandate format for a checkout completion request.', 400);
  }
  const amount = Number(checkout.totals?.find((total) => total.type === 'total')?.amount || 0);
  const mandateReceipt = mandate
    ? verifyAutonomousMandate({ mandate, cart: checkout.line_items, amount, currency: checkout.currency })
    : (officialMandateReceipt
      ? { ...officialMandateReceipt, ...(officialPaymentReceipt ? { paymentMandateId: officialPaymentReceipt.paymentMandateId } : {}) }
      : null);
  const durableMandate = mandate || (officialMandateReceipt ? {
    mandateId: officialMandateReceipt.mandateId,
    agentKeyId: officialMandateReceipt.agentKeyId,
    cartHash: officialMandateReceipt.cartHash,
    currency: officialMandateReceipt.currency,
    amountMax: officialMandateReceipt.amountMax,
    expiresAt: officialMandateReceipt.expiresAt,
    signature: officialMandateReceipt.signature
  } : null);
  ...
  try {
    if (mandateReceipt) await reserveAutonomousMandate({ mandate: durableMandate, checkoutId: checkout.id, admin });
