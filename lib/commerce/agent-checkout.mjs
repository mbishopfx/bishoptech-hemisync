...
export async function getTonePackDelivery({ input, origin = siteOrigin(), supabase, stripe } = {}) {
  ...
  const client = stripeClient(stripe);
  const session = await client.checkout.sessions.retrieve(parsed.checkoutSessionId);
  const sessionPack = session?.metadata?.packSlug || session?.metadata?.planId;
  if (sessionPack !== pack.slug || session.payment_status !== 'paid') {
    throw commerceError('PAYMENT_NOT_VERIFIED', 'The completed payment could not be verified for this tone pack.', 403);
  }

  const purchase = await fulfillTonePackPurchase({ stripeSession: session, supabase: admin });
  ...
  return {
    status: 'paid',
    pack: getPublicTonePack(pack.slug),
    downloadUrl: purchase.bundleUrl || protectedDeliveryUrl,
