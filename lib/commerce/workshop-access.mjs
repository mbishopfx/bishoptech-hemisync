...
export const WorkshopAccessSessionInputSchema = z.object({
...
function assertPaidWorkshopSession(stripeSession) {
  ...
  if (stripeSession?.metadata?.planId && stripeSession.metadata.planId !== 'workshop-24h') {
    throw commerceError('PAYMENT_MISMATCH', 'The checkout session does not match the approved workshop product.', 403);
  }
  ...
  const observedPriceIds = [stripeSession?.metadata?.priceId, lineItemPriceId(stripeSession)]
    ...
    .filter(Boolean);
  if (observedPriceIds.length === 0 || observedPriceIds.some((value) => value !== WORKSHOP_PRICE_ID)) {
    throw commerceError('PAYMENT_MISMATCH', 'The checkout price does not match the approved workshop price.', 403);
  }
}
...
export async function getWorkshopAccessForSession({ sessionId, supabase, stripe, origin = siteOrigin(), secret } = {}) {
  ...
  let session;
  try {
    session = await stripeClient(stripe).checkout.sessions.retrieve(id, { expand: ['line_items'] });
  } catch (error) {
    throw stripeVerificationError(error, 'Workshop payment verification is temporarily unavailable.');
  }
...
export async function revokeWorkshopAccess({ input, supabase, now = new Date(), reason = 'user_requested', secret } = {}) {
  ...
  if (row.status !== 'active') return { revoked: false, status: row.status, accessKeyHint: row.access_key_hint };

  const { data, error } = await admin.from('workshop_access_keys').update({
    status: 'revoked',
    revoked_at: now.toISOString(),
    revoked_reason: String(reason).slice(0, 120),
    updated_at: now.toISOString()
  }).eq('id', row.id).eq('status', 'active').select('status').maybeSingle();
  if (error) throw error;
  if (!data) {
    const current = await readByHash(admin, hashWorkshopAccessKey(parsed.accessKey, secret));
    if (current?.status === 'revoked') return { revoked: true, status: 'revoked', accessKeyHint: current.access_key_hint };
    return { revoked: false, status: current?.status || 'not_active', accessKeyHint: row.access_key_hint };
  }
