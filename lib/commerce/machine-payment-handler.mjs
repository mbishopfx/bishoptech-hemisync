...
// MPP's default HTTP credential field is Authorization. Keep accepting the
// alternate Payment-Authorization field for clients that reserve
// Authorization for an application bearer token. The challenge remains on the
// protocol default so Link Agent Wallet and other standard MPP clients work.
export const MACHINE_PAYMENT_CREDENTIAL_HEADERS = [
  'Payment-Authorization',
  'Authorization'
];

export function createMachinePaymentTransport() {
  ...
  return {
    ...
    getCredential(request) {
      for (const headerName of MACHINE_PAYMENT_CREDENTIAL_HEADERS) {
        ...
        const payment = Credential.extractPaymentScheme(headerValue);
        if (payment) return Credential.deserialize(payment);
      }
      return null;
    }
  };
...
export function createMachinePaymentHandler({
  ...
  const mppx = Mppx.create({
    ...
    transport: createMachinePaymentTransport()
  });
