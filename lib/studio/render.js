    ...
    await new Promise((resolve, reject) => {
      const upload = new tus.Upload(buffer, {
        ...
        onError: (error) => {
          ...
          if (details.includes('413') || details.includes('Maximum size exceeded')) {
            const requiredMb = Math.ceil(buffer.length / (1024 * 1024));
            reject(new Error(`Supabase Global file size limit is below this ${requiredMb} MB master. Raise Storage Settings → Global file size limit to at least 4 GB (a paid Supabase plan is required above 50 MB).`));
            return;
          }
          reject(error);
        },
