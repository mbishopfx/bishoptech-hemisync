...
export async function persistRenderArtifacts({ baseName = 'session', wavBuffer, webmBuffer }) {
  ...
  if (webmBuffer) {
    const storedName = 'master.webm';
    await fs.writeFile(path.join(artifactDir, storedName), webmBuffer);
    files.webm = {
      filename: `${safeBaseName}.webm`,
      contentType: 'audio/webm',
      bytes: webmBuffer.length,
      url: `/api/audio/artifacts/${artifactId}/webm`
    };
  }
