...
export function generateBreathEnvelope(pattern, sampleRate, durationSec, bpm) {
  const totalSamples = Math.floor(sampleRate * durationSec);
  const envelope = new Float32Array(totalSamples);

  let cycleSec;
  if (pattern === 'coherent-5.5') {
    // Allow custom coherent breathing BPM override if provided
    const coherentBpm = Number(bpm) > 0 ? Number(bpm) : 5.5;
    cycleSec = 60 / coherentBpm; // default ~10.9s
