import { dbToGain, lerp } from './utils.js';
...
function resolveCarrierPath(carrierLeftHzFrom, carrierLeftHzTo, shape = null) {
  if (shape && Array.isArray(shape) && shape.length > 0) {
    return shape.map(({ at = 0, hz = carrierLeftHzFrom }) => ({ at, hz }));
  }
  return [
    { at: 0, hz: carrierLeftHzFrom },
    { at: 1, hz: carrierLeftHzTo }
  ];
}
...
export function renderBinauralBed({
  ...
  const startCarrier = Number.isFinite(carrierLeftHzFrom) ? carrierLeftHzFrom : baseLeftHz;
  const endCarrier = Number.isFinite(carrierLeftHzTo) ? carrierLeftHzTo : baseLeftHz;
  const carrierPath = resolveCarrierPath(startCarrier, endCarrier, carrierLeftCurve);
  const maxCarrierAt = Math.max(...carrierPath.map((p) => p.at || 0), 1);
  ...
  for (let i = 0; i < totalSamples; i++) {
    ...
    // carrier left interpolation
    let leftHz = baseLeftHz;
    if (carrierPath.length > 1) {
      const scaledT = norm * maxCarrierAt;
      let idx = 0;
      for (let k = 0; k < carrierPath.length; k++) {
        if ((carrierPath[k].at || 0) <= scaledT) idx = k;
      }
      const p1 = carrierPath[idx];
      const p2 = carrierPath[Math.min(carrierPath.length - 1, idx + 1)];
      const span = Math.max(1e-6, (p2.at || 0) - (p1.at || 0));
      const localT = Math.min(1, Math.max(0, (scaledT - (p1.at || 0)) / span));
      leftHz = lerp(p1.hz ?? baseLeftHz, p2.hz ?? baseLeftHz, localT);
    } else {
      leftHz = lerp(startCarrier, endCarrier, norm);
    }

    const rightHz = leftHz + (hasBinaural ? deltaHz : 0);

    phaseL += (2 * Math.PI * leftHz) / sampleRate;
    phaseR += (2 * Math.PI * rightHz) / sampleRate;
    
    // wrap phase to prevent precision loss over long durations
    if (phaseL > 2 * Math.PI) phaseL -= 2 * Math.PI;
    if (phaseR > 2 * Math.PI) phaseR -= 2 * Math.PI;

    let lSample = hasBinaural ? Math.sin(phaseL) : 0;
    let rSample = hasBinaural ? Math.sin(phaseR) : 0;

    if (hasMonaural) {
      phaseMono += (2 * Math.PI * (leftHz + deltaHz)) / sampleRate;
      if (phaseMono > 2 * Math.PI) phaseMono -= 2 * Math.PI;
      const mono = Math.sin(phaseL) + Math.sin(phaseMono); // assuming baseFreqHz is leftHz
      lSample += 0.5 * mono;
      rSample += 0.5 * mono;
    }

    if (hasIsochronic) {
      phaseIso += (2 * Math.PI * deltaHz) / sampleRate;
      if (phaseIso > 2 * Math.PI) phaseIso -= 2 * Math.PI;
      const gate = 0.5 * (1 + Math.sin(phaseIso));
      lSample *= gate;
      rSample *= gate;
    }
...
export function mixStereo({ targetL, targetR, addL, addR, gain = 1 }) {
  ...
  for (let i = 0; i < frames; i++) {
    targetL[i] += gain * (addL[i] || 0);
    targetR[i] += gain * (addR[i] || 0);
  }
}
