import { renderBinauralBed, mixStereo } from './binaural.js';
import { decodeAudioFile, decodeAudioDataUrl, resampleStereo, trimToFrames } from './import.js';
import { encodeMp3Stereo, encodeWavStereo, encodeWebmStereo } from './encode.js';
import { dbToGain } from './utils.js';
import { applyMasteringChain } from './mastering.js';
...
export async function buildSessionBed({
  ...
  return renderBinauralBed({
    ...
    deltaCurve: deltaCurve ?? focusPreset?.deltaHzPath,
    noise: presetNoise,
    breath,
    modes: presetModes,
    background,
...
export async function encodeOutputs({
  ...
  if (withWebm) {
    try {
      webmBuffer = await encodeWebmStereo({
        left: mastered.left,
        right: mastered.right,
        sampleRate,
        kbps
      ...
      console.warn('WebM encode failed', err);
    }
  }
  ...
  return { wavBuffer, webmBuffer, mp3Buffer, mastering: mastered.metadata };
}
