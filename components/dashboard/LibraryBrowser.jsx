...
function RenderCard({ render, onEditProject }) {
  ...
  return (
    <Card className="rounded-3xl border-white/5 bg-zinc-900/40 p-5 backdrop-blur-xl">
      ...
      <div className="mt-4 grid grid-cols-2 gap-2 text-[10px] font-mono uppercase text-white/30"><div className="rounded-xl border border-white/5 p-3">Duration<p className="mt-1 text-white/65">{Math.round((render.metadata?.durationSec || project?.spec?.durationSec || 0) / 60)} min</p></div><div className="rounded-xl border border-white/5 p-3">Quality<p className="mt-1 text-white/65">24-bit / 48 kHz</p></div></div>
      {render.phase === 'completed' && <div className="mt-4 grid grid-cols-2 gap-2"><button type="button" onClick={() => download('wav')} disabled={busy} className="rounded-xl border border-white/10 px-3 py-2 text-[10px] font-mono uppercase text-white/60">{busy === 'wav' ? 'Opening…' : 'WAV'}</button><button type="button" onClick={() => download('mp3')} disabled={busy} className="rounded-xl border border-white/10 px-3 py-2 text-[10px] font-mono uppercase text-white/60">{busy === 'mp3' ? 'Opening…' : 'MP3'}</button><button type="button" onClick={email} disabled={busy} className="col-span-2 rounded-xl border border-purple-500/20 bg-purple-500/10 px-3 py-2 text-[10px] font-mono uppercase text-purple-200">{busy === 'email' ? 'Sending…' : 'Email Me a Copy'}</button></div>}
