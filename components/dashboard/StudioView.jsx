...
export function StudioView({ initialProject = null, initialRender = null, onChanged, onOpenLibrary, mode = 'studio', seedTone = null }) {
  ...
  const saveProject = async () => {
    ...
    try {
      ...
      setSpec(normalizeClientSpec(data.project.spec));
  ...
  const duplicateProject = async (mode) => {
    ...
    try {
      ...
      const nextSpec = normalizeClientSpec(data.project.spec);
      setSpec(nextSpec);
      setActiveStageId(nextSpec.stages[0]?.id || null);
      setRender(null);
      setDownloads(null);
